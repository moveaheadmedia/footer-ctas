...
use FooterCtas\Common\Traits\Requester;
use FooterCtas\Common\Utils\Errors;
use FooterCtas\Config\Classes;
use FooterCtas\Config\I18n;
use FooterCtas\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'footer_ctas_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'footer_ctas_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'footer-ctas' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Footer CTAs classes are not being loaded by Composer\'s Autoloader' ),
